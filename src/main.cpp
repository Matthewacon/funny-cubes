...
int main() {
  ...
  customCompileShader(GL_VERTEX_SHADER, "../assets/shaders/test.vert",
                      &vertShader);
  customCompileShader(GL_FRAGMENT_SHADER, "../assets/shaders/test.frag",
                      &fragShader);
  ...
  // layout #, vector size, data type, should normalize (0to1), stride, offset
  // from start
  ...
  // Unbind the array buffer since there won't be any operations performed on it
  // anymore
  ...
  // int vertexNewColourLocation = glGetUniformLocation(shaderProgram,
  // "newColour"); double updateTime = 0;
  unsigned int texture = load2DTexture("../assets/images/thonk.png");
